...
The v0.6 GitHub Release includes `mobilelab-android-0.6.0.jar`. Place it under the application's `app/libs` directory and add `implementation(files("libs/mobilelab-android-0.6.0.jar"))`; the JAR targets JVM bytecode 8 and expects the application's normal Kotlin standard library.
